...
def mal_login():
    ...
    verifier = secrets.token_urlsafe(96)   # 128 base64url chars — PKCE-safe
    ...
    # urlencode properly escapes all values — the old manual join would break
    # on any value containing spaces or special characters.
...
def mal_callback():
    # MAL sends ?error=... if the user denied or something went wrong on their end
    ...
    # Retrieve the PKCE verifier saved at login time
    verifier = session.pop("pkce_verifier", "")
    session.pop("oauth_state", None)

    if not verifier:
        # Root cause of the original bug: SECRET_KEY was regenerated on restart,
        # so the session cookie was invalid by the time the callback arrived.
        return redirect("/?mal_error=" + quote(
