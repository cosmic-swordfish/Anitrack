...
# ── PKCE cookie helpers (bypass Flask session for Vercel serverless) ──────────
# Flask sessions can be lost between /mal/login and /mal/callback on Vercel
# because each request may hit a different cold function instance.
# Instead, we store the verifier + state in a plain signed cookie directly.

def _sign(value: str) -> str:
    return hmac.new(_secret.encode(), value.encode(), hashlib.sha256).hexdigest()

def set_pkce_cookie(response, verifier: str, state: str):
    ...
    response.set_cookie(
        "mal_pkce",
        cookie_val,
        max_age=600,          # 10 min — plenty for OAuth round-trip
        httponly=True,
        secure=IS_HTTPS,
        samesite="Lax",
        path="/",
...
def get_pkce_cookie():
    cookie_val = request.cookies.get("mal_pkce", "")
...
def mal_login():
    ...
    # Store in a signed cookie instead of Flask session — survives Vercel cold starts
    ...
    set_pkce_cookie(resp, verifier, state)
    return resp
...
def mal_callback():
    ...
    # Read verifier + state from signed cookie (not Flask session)
    verifier, saved_state = get_pkce_cookie()

    if not verifier or not saved_state:
        return redirect("/?mal_error=" + quote(
            "PKCE cookie missing or expired. Make sure cookies are enabled and try again."))
