...
def al_update(media_id, status=None, progress=None, score=None):
    token = session.get("al_token", {}).get("access_token")
    if not token:
        return False
    ...
    try:
        data = anilist_query(GQL_UPDATE_ENTRY, variables, token=token)
...
def apply_diff_item(diff):
    ...
    if diff.get("action") not in ("add_to_mal", "update"):
...
def sync_diff():
    ...
    try:
        al_token = session.get("al_token", {}).get("access_token")
        al_list  = get_anilist_list(token=al_token)
...
def sync_apply():
    ...
    results = []
    for item in items:
        if not item.get("malId") or item.get("action") not in ("add_to_mal", "update"):
            continue
